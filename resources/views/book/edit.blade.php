...
<div class="max-w-4xl mx-auto py-8">
    ...
    <div class="bg-white shadow-md rounded-xl p-6">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-semibold">Edit Books</h2>
            <a href="{{ url('books') }}" class="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-md text-sm font-medium">
                Back
            </a>
        </div>
        <form action="{{ url('books/'.$book->id.'/edit') }}" method="POST" class="space-y-6">
            @csrf
            {{-- editing --}}
            @method('PUT')
            <div>
